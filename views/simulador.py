...
def mostrar():
    st.title("Simulador de Creditos")
    
    tipo= st.selectbox("tipo de credito", ["Hipotecario", "Vehicular"])
    monto = st.number_input("Monto de Credito", min_value=1000, step=1000)
    plazo = st.slider("Plazo (años)", 1 , 30)
    tasa= st.number_input("Tasa de Interes (%)", min_value=0.1, step=0.1)
